...
int main() {
	...
	al_convert_mask_to_alpha(cometImage, al_map_rgb(0, 255, 0));
	explimage = al_load_bitmap("Explosion.png");
	al_convert_mask_to_alpha(explimage, al_map_rgb(0, 0, 0));
	...
	while (!done) {
		...
		if (render && al_is_event_queue_empty(event_queue)) {
			...
			al_draw_textf(font, al_map_rgb(255, 255, 255), 5, 5, 0, "Player has %i lives left. Score: %i", ship->GetLives(), ship->GetScore());
